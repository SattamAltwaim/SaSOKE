    ...
    if target:
        if model_type == 'mbart_multi_part':
            src2id = {'how2sign': {'body': 'en_ASL', 'lhand': 'en_ASL_lhand', 'rhand': 'en_ASL_rhand'}, 
                    ...
                    'isharah': {'body': 'ar_ssl', 'lhand': 'ar_ssl_lhand', 'rhand': 'ar_ssl_rhand'}}
        else:
            src2id = {'how2sign': {'body': 'en_ASL', 'lhand': 'en_ASL', 'rhand': 'en_ASL'}, 
                    ...
                    'isharah': {'body': 'ar_ssl', 'lhand': 'ar_ssl', 'rhand': 'ar_ssl'}}
    else:
        src2id = {'how2sign': 'en_XX', 'csl': 'zh_CN', 'phoenix': 'de_DE', 'isharah': 'ar_XX'}
    for s in data_src:
        if target:
            token = src2id[s][part]
        else:
...
class Mbart_Based_MLM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if model_type == 'mbart_multi_part':
            new_lang_token = ['en_ASL', 'en_ASL_lhand', 'en_ASL_rhand', 'zh_CSL', 'zh_CSL_lhand', 'zh_CSL_rhand', 'de_DGS', 'de_DGS_lhand', 'de_DGS_rhand', 'ar_ssl', 'ar_ssl_lhand', 'ar_ssl_rhand']
        else:
            new_lang_token = ['en_ASL', 'zh_CSL', 'de_DGS', 'ar_ssl']
        ...
        self.lang_token_ids = list(map(self.tokenizer.convert_tokens_to_ids, ['en_XX', 'zh_CN', 'de_DE', 'ar_XX', '<mask>']+new_lang_token))
