...
class Text2MotionDatasetCB(data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if 'how2sign' in dataset_name or 'isharah' in dataset_name:
            self.data_dir = os.path.join(data_root, split, 'poses')
            # Support both how2sign and isharah CSV naming
            if 'isharah' in dataset_name:
                csv_filename = f'isharah_realigned_{split}_preprocessed_fps.csv'
            else:
                csv_filename = f'how2sign_realigned_{split}_preprocessed_fps.csv'
            self.csv_path = os.path.join(data_root, split, 're_aligned', csv_filename)
            self.csv = pd.read_csv(self.csv_path)
            self.fps = self.csv['fps']
            self.csv['DURATION'] = self.csv['END_REALIGNED'] - self.csv['START_REALIGNED']
            # Removed duration filter - was incorrectly removing valid data
            # self.csv = self.csv[self.csv['DURATION']<30].reset_index(drop=True)
            self.ids = self.csv['SENTENCE_NAME'] #[:200]

            dataset_label = 'isharah' if 'isharah' in dataset_name else 'how2sign'
            print(f'loading {dataset_label} data...', len(self.ids))
            skipped_count = 0
            for idx in tqdm(range(len(self.ids))):
                ...
                if name in bad_how2sign_ids:
                    skipped_count += 1
                    continue
                # Check if pose folder exists before adding
                pose_folder = os.path.join(self.data_dir, name)
                if not os.path.exists(pose_folder):
                    skipped_count += 1
                    continue
                # Determine source: use 'isharah' if dataset_name contains it, otherwise 'how2sign'
                src_label = 'isharah' if 'isharah' in dataset_name else 'how2sign'
                self.all_data.append({'name': name, 'fps': self.csv[self.csv['SENTENCE_NAME']==name]['fps'].item(), 
                                        'text': self.csv[self.csv['SENTENCE_NAME']==name]['SENTENCE'].item(), 'src': src_label})
            if skipped_count > 0:
                print(f'  ⚠ Skipped {skipped_count} entries without pose folders')
            if 'isharah' in dataset_name:
                self.isharah_len = len(self.all_data)
            else:
    ...
    def __getitem__(self, idx):
        ...
        if src == 'how2sign' or src == 'isharah':
            # Isharah uses the same format as how2sign (OSX_params)
            ...
            # Fallback: use text from sample if no loader matches
