...
def load_h2s_sample(ann, data_dir, need_pose=True, code_path=None, need_code=False):
    ...
    # Support both original H2S format and OSX_params format
    # Original: {name}_{frame_id}_3D.pkl
    # OSX_params: frame{frame_id:04d}_person_0_params.pkl
    all_files = sorted([f for f in os.listdir(base_dir) if f.endswith('.pkl')])
    
    if all_files and all_files[0].startswith('frame'):
        # OSX_params format: frame0000_person_0_params.pkl
        frame_list = [os.path.join(base_dir, f) for f in all_files]
    else:
        # Original H2S format
        frame_list = [os.path.join(base_dir, name+'_'+str(frame_id)+'_3D.pkl') for frame_id in range(len(all_files))]
    if fps > 24:
