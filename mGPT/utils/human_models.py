...
import os
SMPLX_AVAILABLE = os.path.exists(human_model_path) and os.path.exists(os.path.join(human_model_path, 'smplx'))

class SMPLX(object):
    def __init__(self):
        if not SMPLX_AVAILABLE:
            print("WARNING: SMPL-X models not found. Visualization will be disabled.")
            self.layer = None
            self.vertex_num = 10475
            self.face = None
            self.shape_param_dim = 10
            self.expr_code_dim = 10
            # Dummy attributes for metrics (not used without SMPL-X)
            self.joint_part2idx = {'body': [], 'lhand': [], 'rhand': [], 'face': []}
            self.vertex_part2idx = {'body': [], 'lhand': [], 'rhand': [], 'face': []}
            self.J_regressor_idx = {'pelvis': 0, 'lwrist': 20, 'rwrist': 21, 'neck': 12}
            self.j14_regressor = None
            self.hand_vertex_idx = {'left_hand': [], 'right_hand': []}
            self.face_vertex_idx = []
...
def get_coord(root_pose, body_pose, lhand_pose, rhand_pose, jaw_pose, shape, expr, remove_lower=True):
    # Return None if SMPL-X not available (not needed for training)
    if not SMPLX_AVAILABLE or smpl_x.layer is None:
        return None, None
