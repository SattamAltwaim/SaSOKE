...
from .load_data import load_h2s_sample, load_csl_sample, load_phoenix_sample, load_isharah_sample
...
class Text2MotionDatasetCB(data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if 'isharah' in dataset_name and self.isharah_root is not None:
            ann_split = split  # always 'train' forced above
    ...
    def __getitem__(self, idx):
        ...
        # Retry loop: skip samples where tokens are missing/corrupted
        for _attempt in range(len(self.all_data)):
            ...
            try:
                if src == 'how2sign':
                    ...
                    _, caption, name, m_tokens = load_isharah_sample(sample, self.isharah_root, need_pose=False, code_path=os.path.join(self.data_root, self.code_path), need_code=True)
                else:
                    m_tokens = None
            except Exception:
                m_tokens = None
            ...
            if m_tokens is not None:
                break
            # Sample is missing/corrupted — try the next one
            data_idx = (data_idx + 1) % len(self.all_data)
